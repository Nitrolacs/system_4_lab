...
// функция для поиска записи по полю предприятие-изготовитель
void search_by_manufacturer() {
    char manufacturer[MAX_LEN]; // строка для хранения данных о предприятие-изготовителе
    struct product p; // структура для хранения данных о продукте
    struct iovec iov[4]; // массив структур iovec для векторного чтения
    int found = 0; // флаг нахождения записи

    printf("Введите предприятие-изготовитель для поиска: ");
    ...
    while (1) { // бесконечный цикл
        ...
        if (strcmp(p.manufacturer, manufacturer) == 0) { // сравнить данные о предприятие-изготовителе с искомым значением
            found = 1; // установить флаг нахождения записи
    ...
    if (!found) { // если флаг не установлен, то сообщить об отсутствии записи
        printf("Не найдено ни одной записи с таким предприятием-изготовителем.\n");
    }
}

// функция для поиска записи по полю тип
void search_by_type() {
    char type[MAX_LEN]; // строка для хранения данных о типе
    struct product p; // структура для хранения данных о продукте
    struct iovec iov[4]; // массив структур iovec для векторного чтения
    int found = 0; // флаг нахождения записи

    printf("Введите тип для поиска: ");
    scanf("%s", type);
    ...
    while (1) { // бесконечный цикл
        ...
        if (strcmp(p.type, type) == 0) { // сравнить данные о типе с искомым значением
            found = 1; // установить флаг нахождения записи
            printf("Найдена запись:\n");
            printf("Предприятие-изготовитель: %s\n", p.manufacturer);
            printf("Тип: %s\n", p.type);
    ...
    if (!found) { // если флаг не установлен, то сообщить об отсутствии записи
        printf("Не найдено ни одной записи с таким типом.\n");
    }
}

// функция для поиска записи по диапазону цены
void search_by_price_range() {
    double min_price, max_price; // переменные для хранения минимальной и максимальной цены
    struct product p; // структура для хранения данных о продукте
    struct iovec iov[4]; // массив структур iovec для векторного чтения
    int found = 0; // флаг нахождения записи

    printf("Введите минимальную цену для поиска: ");
    scanf("%lf", &min_price);
    ...
    while (1) { // бесконечный цикл
        ...
        if (p.price >= min_price && p.price <= max_price) { // проверить попадание цены в заданный диапазон
            found = 1; // установить флаг нахождения записи
            printf("Найдена запись:\n");
            printf("Предприятие-изготовитель: %s\n", p.manufacturer);
            printf("Тип: %s\n", p.type);
    ...
    if (!found) { // если флаг не установлен, то сообщить об отсутствии записи
        printf("Не найдено ни одной записи с такой ценой.\n");
    }
}

// функция для поиска записи по диапазону производительности
void search_by_performance_range() {
    double min_performance, max_performance; // переменные для хранения минимальной и максимальной производительности
    struct product p; // структура для хранения данных о продукте
    struct iovec iov[4]; // массив структур iovec для векторного чтения
    int found = 0; // флаг нахождения записи

    printf("Введите минимальную производительность для поиска: ");
    scanf("%lf", &min_performance);
    printf("Введите максимальную производительность для поиска: ");
    scanf("%lf", &max_performance);
    ...
    while (1) { // бесконечный цикл
        ...
        if (p.performance >= min_performance && p.performance <= max_performance) { // проверить попадание производительности в заданный диапазон
            found = 1; // установить флаг нахождения записи
            printf("Найдена запись:\n");
            printf("Предприятие-изготовитель: %s\n", p.manufacturer);
            printf("Тип: %s\n", p.type);
    ...
    if (!found) { // если флаг не установлен, то сообщить об отсутствии записи
        printf("Не найдено ни одной записи с такой производительностью.\n");
    }
}
...
void print_menu() {
    ...
    printf("2. Удалить запись из файла.\n");
    printf("3. Поискать запись по предприятию-изготовителю.\n");
    printf("4. Поискать запись по типу.\n");
    printf("5. Поискать запись по диапазону цены.\n");
    printf("6. Поискать запись по диапазону производительности.\n");
    printf("7. Вывести все записи из файла.\n");
...
int main() {
    ...
    while (!exit) { // цикл пока флаг не установлен
        print_menu(); // вызвать функцию для вывода меню
        ...
        switch (choice) { // оператор выбора в зависимости от введенного значения
            ...
            case 3: // если пользователь выбрал 3
                search_by_manufacturer(); // вызвать функцию для поиска записи по предприятию-изготовителю
                break;
            case 4: // если пользователь выбрал 4
                modify_record(); // вызвать функцию для поиска записи по типу
                break;
            case 5: // если пользователь выбрал 5
                search_by_price_range(); // вызвать функцию для поиска записи по диапазону цены
                break;
            case 6: // если пользователь выбрал 6
                search_by_performance_range(); // вызвать функцию для поиска записи по диапазону производительности
                break;
